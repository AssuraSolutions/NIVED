...
export default function CustomPrintPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.designNotes.trim()) {
      toast({ title: "Error", description: "Please provide design notes", variant: "destructive" })
      return false
    }
    if (designFiles.length === 0) {
      toast({ title: "Error", description: "Please upload at least one design file", variant: "destructive" })
      return false
    }
    return true
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const mockDesignFileUrls = designFiles.map(
        (file, index) => `https://storage.example.com/designs/${Date.now()}-${index}-${file.name}`,
      )
      ...
      const orderData = {
        ...
        designFiles: mockDesignFileUrls,
...
Design Files: ${designFiles.length} file(s) uploaded

Total: $${(formData.quantity * 25).toFixed(2)}
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold font-playfair text-gray-900 mb-4">Custom Print Service</h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Bring your designs to life! Upload your artwork and we'll create custom printed apparel just for you.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <Card className="rounded-[255px_15px_225px_15px/15px_225px_15px_255px] border-4 border-white shadow-lg">
              <CardHeader>
                <CardTitle className="text-2xl font-playfair">Customer Information</CardTitle>
            ...
            <Card className="rounded-[255px_15px_225px_15px/15px_225px_15px_255px] border-4 border-white shadow-lg">
              <CardHeader>
                <CardTitle className="text-2xl font-playfair">Product Details</CardTitle>
                <CardDescription>Choose your product specifications</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    {COLORS.map((color) => (
                      <div key={color.name} className="flex items-center space-x-2">
                        ...
                        >
                          ...
                          {formData.color === color.name && <Check className="w-4 h-4 text-[#c9a55c]" />}
                        </Label>
                      </div>
                    ))}
                  </RadioGroup>
            ...
            <Card className="rounded-[255px_15px_225px_15px/15px_225px_15px_255px] border-4 border-white shadow-lg">
              <CardHeader>
                <CardTitle className="text-2xl font-playfair">Design Upload</CardTitle>
                <CardDescription>Upload your design files and provide instructions</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="mt-2">
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-600">Click to upload design files or drag and drop</p>
                  ...
                  {designFiles.length > 0 && (
                    <div className="mt-4">
                      <Label className="text-sm font-medium">Uploaded Files ({designFiles.length})</Label>
                ...
                <div>
                  <Label htmlFor="designNotes">Design Notes & Instructions *</Label>
                  <Textarea
                    id="designNotes"
                    name="designNotes"
